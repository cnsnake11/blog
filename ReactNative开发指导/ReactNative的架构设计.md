...
1. flux系列框架干了什么，没干什么，针对开篇提出的问题。
	...
	3. 【解决职责】继续解耦，采用事件机制解决各层之间通信。注意是flux框架的各层之间，不是各组件之间。
...
2. 组件内：组件的子组件可以直接操作其父组件的state或调用其接口等
	3. 子如何获得父的引用：父组件通过子组件的props的parent={this}来传递自己的引用，要求所有组件都必须这样写，parent属性作为内部的关键字存在。`引申问题：如果组件层级关系发生变化，比如从父子变为爷孙，如何设计，才能让代码的调整变得最小`
	4. 这样的方式孙子调用爷爷是这样的，this.props.parent.props.parent，好长，so架构提供mixin，提供this.getParent()方法
1. 兄弟组件间：
	2. 方案1：假设a要调用b，参考第一条的话，其实就是a要改变b的props，那么a只要改b的props的来源即可，一般就是b的父组件的state。如何改变父的state，请参考第二条。
	3. 方案2：利用事件机制，基本同flux架构
	4. 方案3：维护对象树，架构提供页面单例page对象，no no no，就算拿到了对象引用，也改不了他的props
